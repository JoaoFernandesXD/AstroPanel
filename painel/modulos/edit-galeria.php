...
if($tipo == 'editar'){
  ...
  $status = $_POST['status'];
        ...
        if(count($error) == 0) {
          ...
          mysqli_query($conexao, "UPDATE galeria_album SET titulo='$titulo', descricao='$descricao',imagem='$nome_imagem', url='$url', status='$status' WHERE id='$id'");
          header('Refresh:0');
        ...
        echo "<script>alert('Dados atualizados!');</script>";
       mysqli_query($conexao, "UPDATE galeria_album SET titulo='$titulo', descricao='$descricao', url='$url', status='$status' WHERE id='$id'");
        header('Refresh:0');
