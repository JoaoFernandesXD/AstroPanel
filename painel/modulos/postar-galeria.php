...
<?php
  ...
  $status = $_POST['status'];
        ...
        if(count($error) == 0) {
          ...
          mysqli_query($conexao, "INSERT INTO galeria_album (titulo, descricao, data, imagem, autor, url, status) VALUES ('$titulo','$descricao','$data','$nome_imagem','$eu','$url','$status')");
        ...
        echo "<script>alert('Noticia Postada!');</script>";
         mysqli_query($conexao, "INSERT INTO galeria_album (titulo, descricao, data, imagem, autor, url, status) VALUES ('$titulo','$descricao','$data','$eu','$url','$status')");
        header('Location: ?link=galeria'); 
