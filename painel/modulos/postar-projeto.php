...
<?php
  ...
  $foto = $_FILES['imagem'];
        ...
        if(count($error) == 0) {
          ...
          mysqli_query($conexao, "INSERT INTO projeto (titulo, resumo, status, categoria, imagem, data, criador, url, destaque) VALUES ('$titulo','$resumo','$status','$categoria','$nome_imagem','$data','$eu','$url','$destaque')");
...
?>
  ...
  <div id="bx_inicio">Criar projeto</div>
    ...
    <input type="file" name="imagem"><br>
         ...
         <select name="categoria" id="input_padrao">
          <?php while($fetch = mysqli_fetch_array($cad_noticia)){
          echo '<option value="'.$fetch['id'].'">'.$fetch['categoria'].'</option>';
